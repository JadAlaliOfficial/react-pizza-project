// src/features/formVersion/components/fields/rules/DateRuleComponent.tsx

/**
 * Date Rule Component
 *
 * UI for the "date" validation rule
 * Backend rule name: "date"
 *
 * Behavior:
...
export const DateRuleComponent: React.FC<RuleComponentProps> = ({
  rule,
  ruleIndex,
  onEnabledChange,
  onRulePropsChange,
}) => {
  console.debug("[DateRuleComponent] Rendering for rule:", rule.input_rule_id);
  ...
  const handleEnabledChange = (checked: boolean): void => {
    console.debug("[DateRuleComponent] Enabled changed:", checked);
  ...
  return (
    <div className="flex items-center space-x-2 p-3 border rounded-md hover:bg-accent/50 transition-colors">
      <Checkbox
        id={`rule-date-${ruleIndex}`}
        checked={isEnabled}
        onCheckedChange={handleEnabledChange}
      />
      <div className="flex-1">
        <Label
          htmlFor={`rule-date-${ruleIndex}`}
