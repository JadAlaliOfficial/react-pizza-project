// src/features/formVersion/components/fields/rules/UrlRuleComponent.tsx

/**
 * URL Rule Component
 *
 * UI for the "url" validation rule
 * Backend rule name: "url"
 *
 * Behavior:
...
export const UrlRuleComponent: React.FC<RuleComponentProps> = ({
  rule,
  ruleIndex,
  onEnabledChange,
  onRulePropsChange,
}) => {
  console.debug("[UrlRuleComponent] Rendering for rule:", rule.input_rule_id);

  const isEnabled =
    rule.input_rule_id !== null && rule.input_rule_id !== undefined;

  const handleEnabledChange = (checked: boolean): void => {
    console.debug("[UrlRuleComponent] Enabled changed:", checked);
  ...
  return (
    <div className="flex items-center space-x-2 p-3 border rounded-md hover:bg-accent/50 transition-colors">
      <Checkbox
        id={`rule-url-${ruleIndex}`}
        checked={isEnabled}
        onCheckedChange={handleEnabledChange}
      />
      <div className="flex-1">
        <Label
          htmlFor={`rule-url-${ruleIndex}`}
