...
export function TextAreaFieldConfig({
  ...
  const [rowsHint, setRowsHint] = useState<number>(4);
      ...
      <div className="space-y-3">
        ...
        <Alert className="bg-orange-50 border-orange-200">
          ...
          <AlertDescription className="text-xs text-orange-900">
            Multi-line text input. Preserves line breaks and formatting.
            Supports JSON validation for structured data entry.
          </AlertDescription>
        </Alert>
        ...
        <div className="space-y-1.5">
          ...
          <Input
            ...
            max="20"
            value={rowsHint}
            onChange={(e) => setRowsHint(Number(e.target.value) || 4)}
            className="h-9 text-xs"
          />
          <p className="text-[10px] text-muted-foreground">
            💡 Suggested: 3-4 for short text, 6-8 for paragraphs, 10+ for long
            content
          </p>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          />
          <p className="text-[10px] text-muted-foreground">
            💡 Multi-line placeholders supported (use line breaks)
          </p>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <Input
            ...
            min="0"
            value={charLimitHint}
            onChange={(e) => setCharLimitHint(e.target.value)}
            placeholder="e.g., 500, 1000, 5000"
            className="h-9 text-xs"
          />
          <p className="text-[10px] text-muted-foreground italic">
            For display only. Add "max" validation rule later to enforce limit.
          </p>
        </div>
